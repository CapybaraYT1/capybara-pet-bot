...
async def create_clan_finish(message: Message, state: FSMContext):
    ...
    existing = db.get_clan_by_name(name)
    if existing:
...
async def leave_clan_confirm(callback: CallbackQuery):
    ...
    await callback.message.answer("⚠️ Ты уверен что хочешь покинуть клан?", reply_markup=confirm_inline(f"leave_{clan_id}"))
...
async def donate_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("💰 Сколько монет хочешь пожертвовать в казну клана?", reply_markup=back_button())
    await callback.answer()
...
async def donate_finish(message: Message, state: FSMContext):
    ...
    await state.clear()
    await message.answer(f"✅ Ты пожертвовал <b>{amount} монет</b> в казну клана!", reply_markup=clans_menu(), parse_mode="HTML")
...
async def invite_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("📨 Введи <b>ID</b> игрока которого хочешь пригласить:", reply_markup=back_button(), parse_mode="HTML")
    await callback.answer()
...
async def invite_finish(message: Message, state: FSMContext):
    ...
    target = None
    try:
        target = db.get_user(int(message.text.strip()))
    except:
        pass
    ...
    if success:
        ...
        try:
            await message.bot.send_message(target['user_id'], f"📨 Тебя приглашают в клан <b>{clan['name']}</b>!\nПроверь раздел 👥 Кланы → 📨 Приглашения", parse_mode="HTML")
        except:
            pass
        await message.answer(f"✅ Приглашение отправлено!", reply_markup=clans_menu())
    else:
