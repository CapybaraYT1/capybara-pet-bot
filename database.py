...
def equip_item(user_id, item_id):
    ...
    if item['item_type'] == 'weapon':
        cur2.execute("UPDATE users SET equipped_weapon = %s WHERE user_id = %s", (f"{item['item_emoji']} {item['item_name']} (+{item['bonus']}%)", user_id))
    else:
        cur2.execute("UPDATE users SET equipped_armor = %s WHERE user_id = %s", (f"{item['item_emoji']} {item['item_name']} (+{item['bonus']}%)", user_id))
    conn.commit()
    cur.close(); cur2.close(); conn.close()
    return item
